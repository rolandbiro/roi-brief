...
must_haves:
  truths:
    - "Executive summary szekció az áttekintés elején (company_name, campaign_goal, budget_range, campaign_types, target_audience)"
...
6. `EXECUTIVE_SUMMARY_SECTION: SectionDef` — Az áttekintés legelején megjelenő összefoglaló szekció:
   - "Összefoglaló": company_name (Cégnév), campaign_goal (Kampány célja), budget_range (Büdzsé), campaign_types (Kampánytípusok — tömb, CAMPAIGN_TYPE_LABELS-szel formázva), target_audience (Célcsoport)

7. `BASE_SECTIONS: SectionDef[]` — Alap szekciók (mindig megjelennek ha van adat):
...
8. `TYPE_SECTIONS: Record<CampaignType, SectionDef>` — Típusspecifikus szekciók (mezőnevek a tényleges sémafájlokból: @lib/schemas/media-buying.ts, performance.ts, brand.ts, social.ts):
   ...
   - "performance_ppc": "Performance/PPC részletek" — performance_specific.target_cpa (Cél CPA), performance_specific.target_roas (Cél ROAS), performance_specific.conversion_events (Konverziós események), performance_specific.landing_pages (Landing page-ek — tömb!), performance_specific.ad_accounts (Hirdetési fiókok), performance_specific.attribution_model (Attribúciós modell)
   - "brand_awareness": "Brand/Awareness részletek" — brand_specific.brand_lift_target (Brand lift cél), brand_specific.message_recall_target (Üzenetrecall cél), brand_specific.creative_concept (Kreatív koncepció), brand_specific.tonality (Hangvétel/tonalitás), brand_specific.positioning (Pozicionálás), brand_specific.awareness_channels (Awareness csatornák)
   - "social_media": "Social Media részletek" — social_specific.platforms (Platformok), social_specific.organic_paid_mix (Organikus/fizetett arány), social_specific.content_types (Tartalomtípusok), social_specific.community_management (Közösségkezelés), social_specific.influencer_plan (Influencer terv), social_specific.posting_frequency (Posztolási gyakoriság)

   Minden típus szekció condition-je: `(data) => data.campaign_types?.includes("TYPE_KEY")`.

   FONTOS: Ellenőrizd a @lib/schemas/media-buying.ts, performance.ts, brand.ts, social.ts fájlokat implementálás előtt, hogy a mezőnevek egyeznek-e a fentiekkel!

9. `getActiveSections(data: BriefData): Array<{ title: string; fields: Array<{ label: string; value: string }> }>` — Összegyűjti az összes releváns szekciót: ELŐSZÖR az EXECUTIVE_SUMMARY_SECTION, utána BASE_SECTIONS, végül típusspecifikusak. Kiszűri az üres mezőket, és szűri az üres szekciókat.

Exportáld: hasValue, getActiveSections, SectionDef, FieldDef, EXECUTIVE_SUMMARY_SECTION, BASE_SECTIONS, TYPE_SECTIONS.
