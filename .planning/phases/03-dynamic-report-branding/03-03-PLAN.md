...
must_haves:
  truths:
    ...
    - "Az email executive summary szekciót tartalmaz az elején (company_name, campaign_goal, budget_range, campaign_types, target_audience)"
...
3. Implementálj inline hasValue/getNestedValue/formatValue helper-eket (hasonlóan mint a pdf-template-ben). Ez szükséges mert a template inline HTML-t generál.

4. Szekció definíciók: ugyanaz a struktúra mint a brief-sections.ts-ben (EXECUTIVE_SUMMARY + BASE + type-specific). Az EXECUTIVE_SUMMARY szekció az elején: company_name, campaign_goal, budget_range, campaign_types (CAMPAIGN_TYPE_LABELS-szel formázva), target_audience. Duplikálás szükséges mert HTML string-eket generálunk.

5. `generateEmailHtml(data: BriefData): string`:
   ...
   - EXECUTIVE_SUMMARY szekció az elején (kiemelt megjelenéssel)
...
6. Az email formátuma és stílusa a jelenlegi pattern-t követi (inline CSS, table layout), de a szekciók dinamikusak.
